  ...
  <main class="p-5">
    <div id="database-table" class="border">
      <!-- Put database in here-->
        ...
        if(isset($_GET['tables']) and strlen($_GET['tables']) != 0){
            ...
            $sql->execute();
            $deletable = array("creditAcc", "debitAcc", "employee");
            $deleteButton = in_array($_GET['tables'], $deletable);
            ...
            while ($row = $sql->fetch() ){
                ...
                if($deleteButton){
                    echo "<td>"."<input type='button' class='btn btn-danger' onclick='location.href=\"delete.php?tables=".$_GET['tables']."&name=".key($row)."&id=".reset($row)."\";' id='submit-btn' value='Delete' >"."</td>";
                }
                echo "</tr>\n";
            }
            echo "</table>\n";
    ...
    <form>
      ...
      <select id="tables" name="tables">
        ...
        <option value="client_id_to_client_email">Client Email</option>
        <option value="newsletter">Newsletter</option>
        <option value="creditAcc">Credit Accounts</option>
        <option value="debitAcc">Debit Accounts</option>
